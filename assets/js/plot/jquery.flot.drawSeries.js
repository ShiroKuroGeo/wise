(function($){"use strict";function DrawSeries(){function plotLine(datapoints,xoffset,yoffset,axisx,axisy,ctx,steps){var points=datapoints.points,ps=datapoints.pointsize,prevx=null,prevy=null;var x1=0.0,y1=0.0,x2=0.0,y2=0.0,mx=null,my=null,i=0;var initPoints=function(i){x1=points[i-ps];y1=points[i-ps+1];x2=points[i];y2=points[i+1];};var handleSteps=function(){if(mx!==null&&my!==null){x2=x1;y2=y1;x1=mx;y1=my;mx=null;my=null;return true;}else if(y1!==y2&&x1!==x2){y2=y1;mx=x2;my=y1;}
return false;};var handleYMinClipping=function(){if(y1<=y2&&y1<axisy.min){if(y2<axisy.min){return true;}
x1=(axisy.min-y1)/(y2-y1)*(x2-x1)+x1;y1=axisy.min;}else if(y2<=y1&&y2<axisy.min){if(y1<axisy.min){return true;}
x2=(axisy.min-y1)/(y2-y1)*(x2-x1)+x1;y2=axisy.min;}};var handleYMaxClipping=function(){if(y1>=y2&&y1>axisy.max){if(y2>axisy.max){return true;}
x1=(axisy.max-y1)/(y2-y1)*(x2-x1)+x1;y1=axisy.max;}else if(y2>=y1&&y2>axisy.max){if(y1>axisy.max){return true;}
x2=(axisy.max-y1)/(y2-y1)*(x2-x1)+x1;y2=axisy.max;}};var handleXMinClipping=function(){if(x1<=x2&&x1<axisx.min){if(x2<axisx.min){return true;}
y1=(axisx.min-x1)/(x2-x1)*(y2-y1)+y1;x1=axisx.min;}else if(x2<=x1&&x2<axisx.min){if(x1<axisx.min){return true;}
y2=(axisx.min-x1)/(x2-x1)*(y2-y1)+y1;x2=axisx.min;}};var handleXMaxClipping=function(){if(x1>=x2&&x1>axisx.max){if(x2>axisx.max){return true;}
y1=(axisx.max-x1)/(x2-x1)*(y2-y1)+y1;x1=axisx.max;}else if(x2>=x1&&x2>axisx.max){if(x1>axisx.max){return true;}
y2=(axisx.max-x1)/(x2-x1)*(y2-y1)+y1;x2=axisx.max;}};var drawLine=function(){if(x1!==prevx||y1!==prevy){ctx.moveTo(axisx.p2c(x1)+xoffset,axisy.p2c(y1)+yoffset);}
prevx=x2;prevy=y2;ctx.lineTo(axisx.p2c(x2)+xoffset,axisy.p2c(y2)+yoffset);};ctx.beginPath();for(i=ps;i<points.length;i+=ps){initPoints(i);if(x1===null||x2===null){mx=null;my=null;continue;}
if(isNaN(x1)||isNaN(x2)||isNaN(y1)||isNaN(y2)){prevx=null;prevy=null;continue;}
if(steps){var hadMiddlePoint=handleSteps();if(hadMiddlePoint){i-=ps;}}
if(handleYMinClipping())continue;if(handleYMaxClipping())continue;if(handleXMinClipping())continue;if(handleXMaxClipping())continue;drawLine();}
if(mx!==null&&my!==null){initPoints(i);handleSteps();if(!handleYMinClipping()&&!handleYMaxClipping()&&!handleXMinClipping()&&!handleXMaxClipping()){drawLine();}}
ctx.stroke();}
